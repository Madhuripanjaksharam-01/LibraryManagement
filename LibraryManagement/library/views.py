...
from library.models import Books, Course, Issue_book, Student
...
@login_required
@cache_control(no_catch=True, must_revalidate=True, no_store=True)
@never_cache
...
def addbook_func(request):
    if request.method == 'POST':
        ...
        b1.save()
        return render(request, 'add_book.html', {'course': c1, 'msg': 'added successfully', 'res': True})
    else:
        c1 = Course.objects.all()
        return render(request, 'add_book.html', {'course': c1, 'msg': '', 'res': False})
...
def update_book_fun(request, id):
    ...
    return render(request, 'update_book.html', {'books': book, 'course': c1})
...
def get_Student_fun(request):
    ...
    return render(request, 'assign_book.html', {'Book': books, 'Stud': s1})
...
def assignbook_fun(request):
    ...
    if request.method =='POST':
        ...
        i1.save()
        return render(request,'assign_book.html',{'Books':books,'Stud':'',
                                                                 'msg':'assigned successfully','res':True})
    return render(request,'assign_book.html',{'Book':books,'Stud':'','msg':'','res':False})
...
def display_assign_fun(request):
    ...
    return render(request,'display_assign.html',{'issue':i1})
...
def updt_issue_fun(request, id):
    ...
    if request.method == 'POST':
        i1.stud_name = Student.objects.get(stud_name=request.POST['txtName'])
        i1.book_name = Books.objects.get(book_name=request.POST['ddlBookName'])
        i1.start_date = request.POST['txtStartDate']  # Ensure this is in date format
        i1.end_date = request.POST['txtEndDate']  # Ensure this is in date format
        i1.save()
        return redirect('displayassign')  # Correct place for the redirect

    return render(request, 'updt_issue.html', {'Issue': i1, 'Stud': s1, 'Book': books})
